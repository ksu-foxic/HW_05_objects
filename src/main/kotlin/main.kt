data class Post(
    ...
    val date: Int = 1430,//Время публикации записи
...
fun main() {
    ...
    println("Добавлен пост: ${addPost1.string} c ID: ${addPost1.id}")

    val post2 = Post(id = 1, string = "Второй пост", likes = Likes(count = 10))
    val addPost2 = WallService.add(post2)
    println("Добавлен пост: ${addPost2.string} c ID: ${addPost2.id}")

    val updatePost1 = addPost1.copy(string = "Обновленный первый пост", likes = Likes(count = 20))
    val result = WallService.update(updatePost1)
    println("Обновление прошло успешно - $result, новый текст поста ${addPost1.id}: ${updatePost1.string}")
}
